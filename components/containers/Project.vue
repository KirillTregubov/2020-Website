<template>
  <div class="project mt-5 md:mt-6 first:mt-2 overflow-hidden bg-gray-050 rounded-lg shadow-lg">
    <img class="object-cover h-48 md:h-56 xl:h-64 w-full" :src="getImage(name)" :alt="'Image of ' + name">
    <div class="m-4 mb-3">
      <h2 class="text-sm xl:text-base text-gray-500 leading-tight">{{getStack}}</h2>
      <h1 class="mt-3 text-3xl font-semibold leading-none">{{name}}</h1>
      <p class="mt-1 md:mt-2 text-base xl:text-lg text-gray-700">{{description}}</p>
...
a.btn.primary {
  background-color: theme('textColor.gray.900');
...
a.btn.disabled {
  ...
  background-color: theme('textColor.gray.100');
